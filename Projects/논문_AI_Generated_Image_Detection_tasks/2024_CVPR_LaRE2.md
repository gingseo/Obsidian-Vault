...
- <mark style="background: #A6E3A1A6;">Diffusion model의 "닫힌 형태 forward + 단일 스텝 reverse"라는 효율화 전략은, [[2024_ECCV_SR-TOD|SR-TOD]]가 쓰는 self-reconstruction head보다 더 정교한 사전학습 생성 모델 기반 신호 추출 방식으로 확장 가능성이 있어 보인다 — 다만 tiny object detection에는 별도의 대규모 diffusion 사전학습 모델이 필요하다는 진입장벽이 있다.</mark>
