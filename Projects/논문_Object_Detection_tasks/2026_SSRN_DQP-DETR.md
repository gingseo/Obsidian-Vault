...
source: "Projects/_pdf/Object_Detection/DETR/갈래6_쿼리개수/2026_SSRN_DQP-DETR.pdf"
...
- <mark style="background: #FFF3A3A6;">[[2024_ECCV_DQ-DETR|DQ-DETR]], IG-DETR, DQA-DETR 등: density/counting 정보를 query 개수 조정에만 사용하거나 auxiliary supervision으로만 사용.</mark>
...
> [!note]- "값을 맞추는 것"과 "순서를 맞추는 것"의 차이 (RCS가 정확히 뭘 다르게 학습시키는가)
> **Pixel-level regression(`L_den`, ADPG가 D를 학습할 때 쓰는 것)**: "이 픽셀의 예측 밀도값이 정답 밀도값과 최대한 가까워지도록" 오차를 줄인다.
> ```
> L_den = (D_predicted - D_gt)²
> ```
> 이건 "값 자체"를 맞추려는 학습이다. 그런데 값 자체가 정확해도, 그 값들을 순서대로 나열했을 때 top-K를 뽑는 데는 도움이 안 될 수 있다 — 예측값이 실제값보다 전체적으로 낮게 나오는 편향이 있어도 상대적 크기 순서만 맞으면 top-K 선택엔 문제없고, 반대로 절대값 오차는 작아도 순서가 뒤바뀌면 top-K는 완전히 틀릴 수 있다.
>
> **RCS(`L_rank`)는 값이 아니라 순서를 직접 학습시킨다.**
> ```
> q[I_pos] - q[I_neg] ≥ margin   →  loss = 0 (이미 잘 벌어져 있음, 벌점 없음)
> q[I_pos] - q[I_neg] < margin   →  loss > 0 (충분히 안 벌어짐, 벌점 발생)
> ```
> 이 loss는 "고우선순위 토큰의 값이 얼마여야 한다"는 절대값을 요구하지 않는다. 오직 "고우선순위 토큰(`I_pos`, GT 밀도 기준으로 확실히 높은 토큰=대체로 실제 객체 위치)의 예측 priority가, 저우선순위 토큰(`I_neg`, GT 기준 확실히 낮은 토큰=대체로 배경)의 예측 priority보다 최소 margin만큼은 커야 한다"는 상대적 관계만 요구한다.
>
> **역전파로 실제 일어나는 일**: `L_rank > 0`이면(모델 예측 순위가 정답 순위와 충분히 안 벌어져 있으면), loss를 줄이는 방향으로 `q[I_pos]`는 커지고 `q[I_neg]`는 작아져야 한다. `q_i = s_i · d̂_i`이므로 이 신호는 `s_i`(encoder classification branch)와 `d̂_i`(ADPG의 density 예측)를 만드는 파라미터 둘 다로 역전파되어, "실제 객체가 있는 토큰의 priority는 높이고, 배경 토큰의 priority는 낮추는" 방향으로 두 모듈을 동시에 갱신한다 — density map의 픽셀값을 맞추려는 게 아니라, top-K 선택이라는 최종 목적에 정확히 맞춰서 학습 신호를 주는 것.
>
> | | Pixel-level regression(`L_den`) | Token-level 순위 직접 감독(RCS, `L_rank`) |
> |---|---|---|
> | 무엇을 맞추려 하나 | 각 픽셀의 밀도 "값" 자체 | 고/저 우선순위 토큰 간 "상대적 순서" |
> | Loss가 0이 되는 조건 | 예측값 = 정답값 | 예측 pos 토큰 priority - neg 토큰 priority ≥ margin (값 자체는 안 맞아도 됨) |
> | top-K 선택과의 관계 | 간접적(값이 맞으면 순서도 맞을 것이라 "기대") | 직접적(애초에 top-K가 맞게 뽑히도록 순서 자체를 학습) |
...
### 생각할 점
- <mark style="background: #A6E3A1A6;">이 논문은 이 위키의 dynamic query DETR 계열([[2024_ECCV_DQ-DETR|DQ-DETR]], Density-Aware DETR, IG-DETR, DQA-DETR) 중 density 정보를 가장 포괄적으로 활용한다 — encoder 표현 강화(BCME), query 개수 추정, 개별 토큰 순위 결정(RCS) 세 지점 모두에 관여시킨 유일한 사례다.</mark>
...
- <mark style="background: #A6E3A1A6;">RCS의 "GT로 참조 우선순위를 만들고 margin ranking loss로 감독" 방식은, [[2026_TIP_Unc-SOD|Unc-SOD]]의 instance-level uncertainty 기반 접근과 결합해 "밀도 기반 순위"와 "불확실성 기반 순위"를 동시에 반영하는 하이브리드 priority를 만들어볼 여지가 있다.</mark>
- <mark style="background: #A6E3A1A6;">Query priority가 confidence·density의 단순 곱이라는 한계(Discussion 참고)를 보완하려면, [[2026_JSTARS_DQA-DETR|DQA-DETR]]의 attention 기반 병합처럼 고립된 저밀도 고신뢰 토큰을 별도 경로로 구제하는 메커니즘을 추가하는 방향도 검토할 만하다.</mark>
...
# 관련 문서
- 비교: [[Object_Detection_Approaches]] — dynamic query DETR 계열 중 density 정보를 가장 포괄적으로 활용하는 사례.

# 읽어볼 만한 논문
- 참고문헌 기반: Y.-X. Huang, H.-I. Liu, H.-H. Shuai, W.-H. Cheng, "DQ-DETR: DETR with dynamic query for tiny object detection" (ECCV 2024) [24] — 이미 위키에 있음: [[2024_ECCV_DQ-DETR|DQ-DETR]]. 이 논문이 직접 비교 대상으로 삼는 dynamic query DETR의 원조.
