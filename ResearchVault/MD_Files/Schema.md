...
## Task를 가로지르는 개념
task 경계와 무관하게 여러 분야에서 반복 등장하는 개념·신호·메커니즘을 모은다. 각 항목은 그 개념이 등장하는 task/논문을 함께 적어, task가 다른 논문끼리도 이 개념 하나로 서로 찾아갈 수 있게 한다.
- **<개념 이름>** — [[개념-문서]](있으면 링크): 등장하는 task·논문들과, 같은 개념이 어떻게 변형되어 쓰이는지 한두 줄.
