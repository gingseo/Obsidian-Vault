...
# 관련 개념
- [[latent-reconstruction-error]] — "재구성이 어려운 정도를 판별 신호로 쓴다"는 원리를 공유하는 다른 도메인(AI 생성 이미지 탐지)의 유사 기법. Feature map→원본 이미지 재구성(이 개념)과 사전학습 diffusion model의 노이즈 예측 오차(LaRE)라는 재구성 대상의 차이가 있다.
