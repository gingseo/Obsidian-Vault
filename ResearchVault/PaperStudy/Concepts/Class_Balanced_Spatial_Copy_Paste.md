...
# 등장 논문
- [[YOFOR]] — 원조. Class Balance Module(CBM)로 이 개념을 제안. VisDrone에서 7% 미만 비율의 클래스를 tail class로 식별, 좌우 조건부 복제(본문 3.3절은 박스 너비 1/4을 복제 거리로 명시하나, Table 9 ablation은 별도 "Distance" 배율 0/0.5/1/2 중 1배를 최적으로 보고해 두 서술의 정확한 관계가 원문에서 조율되지 않음). Random copy-paste 대비 일관된 우위를 실험으로 확인(Table 10).
