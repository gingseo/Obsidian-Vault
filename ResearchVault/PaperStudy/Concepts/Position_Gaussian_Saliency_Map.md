...
# 등장 논문
- [[2025_CVPR_Feature_Info_Driven_Gaussian|Feature_Info_Driven_Gaussian]] — 각 GT box를 중심 (x,y), 공분산 diag((w/α)²,(h/α)²)인 Gaussian으로 모델링(α는 very tiny/tiny/small/general 크기별로 4/6/8/10 다르게 적용해 tiny일수록 뾰족한 분포). N개 인스턴스의 Mixture를 합산·threshold 후처리해 GT map을 만들고, multi-scale network(P2~P4)가 이를 예측하도록 지도학습. 예측된 맵을 feature에 `y⊗(1+M_pd)` 형태로 곱해 tiny object 영역의 feature activation을 강화하는 데 사용.
