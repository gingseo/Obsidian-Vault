...
# 등장 논문
- [[2025_ESWA_LSOD-YOLO|LSOD-YOLO]] — YOLOv8s에 처음 이 기법을 도입. P5 검출 헤드를 제거하고 P2 헤드를 추가하며 P3-P2 간 cross-layer connection을 결합. Ablation에서 LCOR 제거 시 파라미터가 3.8M→12.3M로 급증하고 mAP0.5가 37.0%→35.8%로 하락해, 경량화 효과의 대부분이 이 모듈에서 나옴을 확인. cross-layer connection만 제거한 변형(파라미터 3.7M, mAP0.5 36.5%)과 비교해 connection 자체의 기여(+0.5%p)도 별도로 검증됨.
...
- 현재로선 [[2025_ESWA_LSOD-YOLO|LSOD-YOLO]] 한 편에서만 제안된 기법이며, 다른 논문에서의 재사용 사례는 아직 확인되지 않음.
