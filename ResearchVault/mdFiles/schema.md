...
raw/
  ...
  <task>/                처리 완료 후 이동되는 task별 폴더 (예: object-detection/, anomaly-detection/)
wiki/
  papers/
    <task>/                raw/<task>/와 동일한 task 폴더 구조. 논문 1편 = 파일 2개(분석 노트 + 원문 요약)
...
- `<task>` 폴더명은 논문의 목적/과제를 가리키는 영문 kebab-case (예: `object-detection`, `anomaly-detection`, `segmentation`, `loss-function`). 기존에 없던 task면 새 폴더를 만든다. 폐쇄 목록이 아니다 — 논문을 보고 적절한 task가 없으면 새로 만든다.
...
- `wiki/` 파일명은 영문 kebab-case. 예: `wiki/papers/attention-is-all-you-need.md`, `wiki/concepts/rotary-positional-embedding.md`
...
- 각 문서 frontmatter의 `tags`는 소문자 kebab-case로 통일한다.
...
논문 1편당 파일 **2개**: 분석 노트(`<slug>.md`)와 원문 요약(`<slug>-source.md`). 하나로 합치지 않는다 — "원문을 그대로 옮긴 것"과 "내가 분석한 것"은 읽는 목적이 달라서, 분석만 보고 싶을 때 번역 전문까지 스크롤하게 만들면 안 된다.

파일명: `wiki/papers/<task>/<논문-슬러그>.md` / `wiki/papers/<task>/<논문-슬러그>-source.md` (슬러그는 예: 저자 성 + 핵심 키워드, 또는 잘 알려진 약칭. 리네임한 PDF의 제목 부분을 재사용해도 된다)

**슬러그 대소문자 규칙**: 논문에 저자가 스스로 붙인 short title(예: "QueryDet: Cascaded Sparse Query for..."처럼 제목 앞부분에 오는 약칭)이 있으면, 그 표기를 대소문자까지 정확히 그대로 슬러그로 쓴다 (`FANet`, `LSOD-YOLO`, `QueryDet`, `RS-TOD`, `UAV-DETR`, `Unc-SOD`, `ReContrast`처럼). 전부 소문자로 뭉뚱그리지 않는다 — 실제 논문에서 쓰는 표기가 그 자체로 식별성을 갖기 때문이다(예: `ReContrast`를 `recontrast`로 쓰면 다른 논문 제목의 일부처럼 보일 수 있다). 원문에 명시적 short title이 없으면(예: 제목이 일반 서술형 문장인 논문) 슬러그는 소문자 kebab-case로 짓는다. 애매하면 PDF를 다시 확인해 저자가 스스로 어떻게 부르는지(본문에서 반복 사용하는 모델/프레임워크 이름) 찾아보고, 그래도 불명확하면 사용자에게 묻는다 — 나중에 슬러그를 바꾸면 위키 전체의 `[[링크]]`를 전수 갱신해야 하는 비용이 크므로 처음에 정확히 정하는 편이 낫다.
...
`<slug>-source.md`(원문 요약 파일)의 frontmatter는 최소한으로 둔다:
...
원문 요약(Abstract/Introduction/Main Contribution/Conclusion 번역): [[<slug>-source]]
...
### 본문 구성 — 원문 요약 (`<slug>-source.md`)

```markdown
[[<slug>]]의 원문 요약. 분석·평가는 담지 않는다 — 순수 번역/발췌.

# Abstract
논문 Abstract 전문을 한국어로 번역. 의역보다 원문 충실도를 우선한다.

# Introduction

이 논문을 분석 노트 없이 이 파일만 읽어도 흐름이 잡히도록, 문단마다 그 문단의 역할을 나타내는 짧은 소제목(`####`)을 달고, 그 아래 1문장(원칙적으로 20단어/40자 내외)으로 압축한다. 소제목은 그 문단이 실제로 하는 역할을 가리키는 말로 짓는다 — 예: `기존 연구 한계 지적`, `이 논문이 다루는 문제`, `제안 방법 개요`, `실험 결과 요약`, `기여 요약`. 문단 수만큼 반복한다. "1문단:", "2문단:" 같은 순번 라벨은 쓰지 않는다 — 소제목 자체가 그 문단의 정체를 알려줘야 한다.

예:
#### 기존 연구 한계 지적
기존 UAV-OD 알고리즘은 NMS·anchor box 등 수작업 튜닝에 의존해 배포 복잡도가 높다.

#### 개선할 부분
DETR 계열은 자연 이미지 기준으로 설계되어 UAV 특유의 극소형 객체·가림에는 그대로 적용하기 어렵다.

# Main Contribution
저자가 Introduction 끝에서 명시한 contribution을 번호 목록으로, 원문 그대로(재해석 없이) 옮긴다. 항목당 1~2줄로 압축 — 원문이 한 항목에 여러 문장을 썼어도 핵심만 남긴다.

# Conclusion
논문 Conclusion 전문을 한국어로 번역. Abstract와 마찬가지로 원문 충실도를 우선한다.
```

원문 요약 파일에는 Highlightr 하이라이트를 적용하지 않는다(아래 하이라이트 규칙 참고). Abstract/Conclusion은 "전문 번역"이 원칙이므로 문단을 추가로 쪼개지 않지만, 원문 자체가 매우 길면(150단어 이상 문단) 의미가 끊기지 않는 선에서 문단을 나눠 가독성을 확보해도 된다 — 다만 문장을 생략하거나 요약하지 않는다(그건 번역이 아니라 요약이 되어버림).
...
- `<slug>-source.md`(원문 요약 파일) 전체에는 하이라이트를 적용하지 않는다 — 원문을 그대로 옮기거나 번역한 것이지 내 분석이 아니므로, "이게 핵심이다"라는 판단을 얹지 않는다.
...
파일명: `wiki/comparisons/<비교-주제-슬러그>.md`
...
### Task MOC — `wiki/moc/<task>-moc.md`

`raw/`, `wiki/papers/`에 존재하는 task 폴더마다 하나씩 만든다 (예: `wiki/moc/small-object-detection-moc.md`).

파일명: `wiki/moc/<task>-moc.md`
...
### 갱신 규칙
- 새 논문이 `wiki/papers/<task>/`에 추가될 때마다, 해당 `<task>-moc.md`가 없으면 새로 만들고, 있으면 "지금까지 다룬 흐름"·"이 분야를 관통하는 개념" 섹션을 갱신한다 (아래 워크플로우 8번 참고).
...
4. `wiki/papers/<task>/<slug>-source.md`를 먼저 만든다 — Abstract/Introduction(문단별 소제목+한 문장)/Main Contribution/Conclusion을 원문 요약 템플릿대로 채운다. 이 파일은 PDF 원문을 그대로 옮기는 작업이라, 뒤에 이어지는 분석의 기초가 된다.
5. `wiki/papers/<task>/<slug>.md`(분석 노트)를 만든다. `source`에 이동 후 PDF 경로(`raw/<task>/<리네임된 파일명>`)를 채우고, `task`, `direction` 속성을 채운다. `status`는 `read`(아직 내 연구 관점 리뷰 전) 또는 `reviewed`(평가까지 마침)로 판단해 채운다. `user_read: false`, `added`(PDF의 `raw/inbox/` 진입 날짜, mtime 기준)도 함께 채운다. `jcr_quartile`은 위 Frontmatter 절의 규칙대로 채운다 — 학회가 명백한 top-tier면 `Q1`, 저널이거나 등급을 모르면 `null`로 두고 나중에 사용자에게 물어볼 목록에 추가한다(추측 금지). "한 줄 요약" 바로 아래에 `[[<slug>-source]]` 링크를 반드시 넣는다. 이하 문제 정의부터는 분석 노트 템플릿(콜아웃·bullet 규칙 포함)을 따른다.
6. `grep -r "#pending:<이번 논문의 슬러그>" wiki/`를 실행해서, 기존 노트 중 이 논문을 미완성 링크(`#pending:` 마커)로 남겨둔 곳이 있는지 확인한다. 있으면 해당 문장을 실제 `[[wiki-link]]`로 갱신하고 마커를 지운다.
7. 논문에서 "독립적으로 설명할 가치가 있는" 개념/기법(핵심 기여인 기법·아이디어)이 있는지 판단한다 — 재사용 여부와 무관하게, 이 논문 1편만 보고 판단한다.
...
8. 이번 논문이 기존 논문과 비교할 가치가 있으면(같은 문제, 같은 벤치마크, 직접적 후속작 등) `wiki/comparisons/`에 문서를 만들거나 기존 비교 문서를 갱신한다.
9. comparison만 애매하면 만들지 않는다 — 나중에 다른 논문이 들어와 근거가 쌓이면 그때 만든다. (concept은 위 7번 기준으로 그때그때 판단한다.)
10. `wiki/moc/<task>-moc.md`를 갱신한다 — 없으면 새로 만들고, 있으면 "지금까지 다룬 흐름"에 이번 논문을 추가하고 다른 논문과의 관계를 서술한다. 새로 만든 concept/comparison이 있으면 해당 섹션에도 링크를 추가한다. task 폴더가 이번에 처음 생겼다면 `wiki/moc/000-home.md`의 "Task별 MOC" 목록에도 추가한다.
11. 이번 논문이 다루는 내용 중, 아직 위키에 없는 다른 논문을 언급하며 나중에 링크할 대상으로 남겨야 하는 경우(예: 비교 대상으로 인용되지만 PDF가 없는 논문) `#pending:<그-논문-슬러그>` 마커를 붙여 남긴다.
12. 논문 노트의 "읽어볼 만한 논문" 섹션을 채운다("읽어볼 만한 논문" 작성 규칙 참고). 여기 추가한 항목은 `wiki/reading-list.md`의 해당 task 섹션에도 동일하게 추가한다.
13. 이번에 처리한 논문이 `wiki/reading-list.md`에 이미 있던 항목이라면(즉 예전에 추천되어 대기 중이던 논문을 지금 실제로 읽은 것이라면), 그 항목을 `wiki/reading-list.md`에서 삭제한다.
14. 이번에 처리한 논문 중 `jcr_quartile`이 `null`로 남은 저널 논문(등급 확인 필요)이 있으면, 처리 완료 보고의 마지막에 "JCR 등급을 확인해 주세요"라는 목록으로 venue와 함께 사용자에게 물어본다. 사용자가 답을 주면 해당 논문 노트의 `jcr_quartile`을 갱신한다.
