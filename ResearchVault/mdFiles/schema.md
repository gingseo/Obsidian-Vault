...
9. `wiki/moc/<task>-moc.md`를 갱신한다 — 없으면 새로 만들고, 있으면 "지금까지 다룬 흐름"에 이번 논문을 추가하고 다른 논문과의 관계를 서술한다. 새로 만든 concept/comparison이 있으면 해당 섹션에도 링크를 추가한다. task 폴더가 이번에 처음 생겼다면 `wiki/moc/000-home.md`의 "Task별 MOC" 목록에도 추가한다.
